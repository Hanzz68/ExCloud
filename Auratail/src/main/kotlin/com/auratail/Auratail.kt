...
class Auratail : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        return newHomePageResponse(HomePageList("Terbaru", items))
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val episodes = episodeSelectors.flatMap { sel ->
            doc.select(sel).mapNotNull { element ->
                ...
                val number = Regex("""\d+""").find(element.text())?.value?.toIntOrNull() ?: return@mapNotNull null
                number to newEpisode(href) { this.name = "$title Episode $number" }
            }
        }.sortedBy { it.first }

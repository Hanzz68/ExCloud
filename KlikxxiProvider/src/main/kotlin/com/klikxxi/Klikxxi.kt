...
open class Klikxxi : MainAPI() {
    ...
    private fun Element.toRecommendResult(): SearchResponse? {
        ...
        val posterUrl = fixUrlNull(
            selectFirst("a > img")?.attr("data-src")
                ?: selectFirst("a > img")?.attr("data-lazy-src")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = document.selectFirst("h1.entry-title, div.mvic-desc h3")?.text()
            ?.substringBefore("Season")?.substringBefore("Episode")
            ?.substringBefore("(")?.trim().orEmpty()
        val poster = fixUrlNull(
            document.selectFirst("figure.pull-left img")?.attr("data-src")
                ?: document.selectFirst("figure.pull-left img")?.attr("data-lazy-src")
        ...
        val trailer = document.selectFirst("div.rll-youtube-player iframe")?.attr("src")
            ?: document.selectFirst("div.rll-youtube-player")?.attr("data-lazy-src")
        ...
        val data = if (tvType == TvType.TvSeries) {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val iframe = document.selectFirst("iframe[src*='stream']")?.attr("src")
        if (iframe != null) loadExtractor(fixUrl(iframe), mainUrl, callback)
        return true
    }
}
